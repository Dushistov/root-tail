...
static Window __ToonGetKDEDesktop(Display *display, int screen, Window window,
                                  Atom atom, char *atomname, int depth)
{
    ...
    if (XFetchName(display, window, &name)) {
        if (strcasecmp(name, "KDE Desktop") == 0) {
            ...
            if (XGetWindowProperty(display, window, atom, 0, 1, False, XA_ATOM,
                                   &actual_type, &actual_format, &nitems,
                                   &bytesafter, (unsigned char **)&wintype)
                    == Success
                && wintype) {
    ...
    if (go_deeper
        && XQueryTree(display, window, &rootReturn, &parentReturn, &children,
                      &nChildren)) {
        int i;
            ...
            if (depth == 2) {
                ...
                if (XGetWindowAttributes(display, children[i], &attributes)) {
                    if (attributes.width >= DisplayWidth(display, screen) / 2
                        ...
                        = __ToonGetKDEDesktop(display, screen, children[i],
                                              atom, atomname, depth + 1))) {
                break;
            }
        }
        XFree((char *)children);
...
static Window __ToonGetNautilusDesktop(Display *display, int screen,
                                       Window window, int depth)
{
    Window rootReturn, parentReturn, *children;
        ...
        for (i = 0; i < nChildren; ++i) {
            ...
            if (XGetWindowAttributes(display, children[i], &attributes)) {
                if (attributes.width == DisplayWidth(display, screen)
                    ...
                    winreturn = __ToonGetNautilusDesktop(
                        display, screen, children[i], depth + 1);
                }
            }
        }
        XFree((char *)children);
...
Window ToonGetRootWindow(Display *display, int screen, Window *clientparent)
{
    ...
    *clientparent = root;
    ...
    if (XGetWindowProperty(display, root, NAUTILUS_DESKTOP_WINDOW_ID, 0, 1,
                           ...
                           &nitems, &bytesafter, (unsigned char **)&toplevel)
            == Success
        && toplevel) {
        /* Nautilus is running */
        background = __ToonGetNautilusDesktop(display, screen, *toplevel, 0);
        XFree((char *)toplevel);
        if (background) {
    ...
    if (!background
        && XQueryTree(display, root, &rootReturn, &parentReturn, &children,
                      &nChildren)) {
        int i;
        ...
        for (i = 0; i < nChildren && !background; ++i) {
            ...
            if (XGetWindowProperty(display, children[i], __SWM_VROOT, 0, 1,
                ...
                Atom *tmpatom;
                if (XGetWindowProperty(display, root, _NET_SUPPORTED, 0, 1,
                    ...
                    XFree((char *)tmpatom);
                    if (XGetWindowProperty(display, root, _NET_VIRTUAL_ROOTS, 0,
                                           1, False, XA_WINDOW, &actual_type,
                                           &actual_format, &nitems, &bytesafter,
                                           (unsigned char **)&tmpwindow)
                            != Success
                        || !tmpwindow) {
                        /* Must be KDE 2.1+ */
                ...
                XFree((char *)newroot);
            } else if ((background = __ToonGetKDEDesktop(
                            display, screen, children[i], _NET_WM_WINDOW_TYPE,
    ...
    if (!background) {
        ...
        if (XGetWindowProperty(display, root, _WIN_WORKSPACE, 0, 1, False,
                               XA_CARDINAL, &actual_type, &actual_format,
                               &nitems, &bytesafter,
                               (unsigned char **)&workspace)
                == Success
            && workspace) {
            /* Found a _WIN_WORKSPACE property - this is the desktop to look
             * for. For now assume that this is Enlightenment. We're looking for
             * a child of the root window that has an ENLIGHTENMENT_DESKTOP atom
             * with a value equal to the root window's _WIN_WORKSPACE atom. */
            Atom ENLIGHTENMENT_DESKTOP
                = XInternAtom(display, "ENLIGHTENMENT_DESKTOP", False);
            /* First check to see if the root window is the current desktop...
             */
            if (XGetWindowProperty(display, root, ENLIGHTENMENT_DESKTOP, 0, 1,
             ...
             * the current desktop */
                ...
                for (i = 0; i < nChildren; ++i) {
                    if (XGetWindowProperty(
                            display, children[i], ENLIGHTENMENT_DESKTOP, 0, 1,
                            False, XA_CARDINAL, &actual_type, &actual_format,
                            &nitems, &bytesafter, (unsigned char **)&desktop)
                            == Success
                        && desktop && *desktop == *workspace) {
